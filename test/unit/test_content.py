...
class TestImportFile:
    ...
    def test_unknown_missing_input_type(self, mock_open, mock_exists, mock_is_file):
        with pytest.raises(
            ...
            ImportFile(
                "test.unknown", separator=",", content_type=ImportContentType.CSV
            )
    ...
    def test_unknown_missing_content_type(self, mock_open, mock_exists, mock_is_file):
        with pytest.raises(
            ...
            ImportFile("test.unknown", separator=",", input_type=ImportType.DELIMITED)
...
class TestRdfRaw:
    ...
    def test_with_content_type_encoding(self):
        m = GraphRaw(
            "some_data", "test.turtle", GraphContentType.TURTLE, ContentEncoding.GZIP
        )
        assert m.name == "test.turtle"
        assert m.content_type == ContentType.TURTLE.value
        assert m.content_encoding == "gzip"
...
class TestImportUrl:
    ...
    def test_with_override(self):
        m = ImportUrl(
            "https://example.org/hello/test.txt",
            input_type=ImportType.JSON,
            separator=",",
            content_type=ImportContentType.CSV,
            content_encoding=ContentEncoding.GZIP,
    ...
    def test_unknown_missing_separator(self):
        with pytest.raises(
            ...
            ImportUrl(
                "https://example.org/hello/test.ttl",
                input_type=ImportType.DELIMITED,
                content_type=ImportContentType.CSV,
            )
    ...
    def test_unknown_missing_input_type(self):
        with pytest.raises(
            ...
            ImportUrl(
                "https://example.org/hello/test.unknown",
                separator=",",
                content_type=ImportContentType.CSV,
            )
    ...
    def test_bad_args(self):
        with pytest.raises(
            ...
            ImportUrl(
                "https://example.org/hello/test.csv", "test", "test", "test", "\n"
            )
...
class TestMappingUrl:
    ...
    def test_with_override(self):
        m = MappingUrl(
            "https://example.org/hello/test.turtle", syntax=MappingSyntax.SMS2
        )
        assert m.name == "test.turtle"
        assert m.mapping_syntax() == MappingSyntax.SMS2.value
        assert not m.is_result_file
