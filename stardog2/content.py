...
import requests
from pydantic import HttpUrl, FilePath, ValidationError, BaseModel
from pydantic.error_wrappers import ErrorWrapper

from stardog2.utils.pydantic import sd_validate_arguments


# we want to add the class above to globals, but since we do not refer to them, you get a warning,
# the line below is a hack to remove that warning


def smart_str_validate_arguments(f):
...
class AskContentType(str, Enum):
    BOOLEAN = "text/boolean"
    SPARQL_JSON = "application/sparql-results+json"
    SPARQL_XML = "application/sparql-results+xml"


class ImportContentType(str, Enum):
    CSV = "text/csv"
    TSV = "text/tab-separated-values"
    JSON = "application/json"


class MappingSyntax(str, Enum):
    SMS2 = "SMS2"
    R2RML = "R2RML"
    STARDOG = "STARDOG"
...
class Content:
    ...
    def __init__(self, name: Path):
        ...
        if classname == "Content":
            raise ValidationError(
                [
                    ErrorWrapper(
                        TypeError("This class should not be instantiated directly"),
        ...
        if (
            not self.is_mapping_file
            and not self.is_result_file
            and not self.is_add_file
            and not self.is_import_file
        ):
            raise ValidationError(
                [
                    ErrorWrapper(
    ...
    def mapping_syntax(self) -> Optional[str]:
    ...
    def has_mapping_suffix(self, path: Path = None) -> bool:
    ...
    def has_rdf_suffix(self, path: Path = None) -> bool:
        path = path if path is not None else self._name
    ...
    def has_import_suffix(self, path: Path = None) -> bool:
        path = path if path is not None else self._name
    ...
    def has_type_suffix(self, path: Path = None) -> bool:
        path = path if path is not None else self._name
    ...
    def has_encoding_suffix(self, path: Path = None) -> bool:
        path = path if path is not None else self._name
...
class _MappingMixIn:
    def _validate(self, name: Path, syntax: MappingSyntax | None):
    ...
    def _validate(
        ...
        if not self.has_rdf_suffix(name) and content_type is None:
            raise ValidationError(
                [
                    ErrorWrapper(
                        TypeError("must be provided for unknown rdf file extension"),
                        loc="content_type",
                    )
                ],
                Init,
    ...
    def _validate(
        ...
        if not self.has_import_suffix(name):
            ...
            if content_type is None:
                errors.append(
                    ErrorWrapper(
                        TypeError("must be provided for unknown import extension"),
                        loc="content_type",
                    )
                )
...
class File(Content):
    @smart_str_validate_arguments
    def __init__(self, file: Union[FilePath, str]):
...
class GraphFile(File, _RdfMixIn):
    ...
    def __init__(
        ...
        content_type: GraphContentType = None,
        content_encoding: ContentEncoding = None,
    ):
...
class MappingFile(File, _MappingMixIn):
    ...
    @smart_str_validate_arguments
    def __init__(self, file: Union[FilePath, str] | str, syntax: MappingSyntax = None):
        """Initializes a File object.
...
class ImportFile(File, _ImportMixIn):
    ...
    def __init__(
        ...
        Examples:
          >>> ImportFile('data.tsv')
          >>> ImportFile('data.txt',input_type=ImportType.DELIMITED, separator="\t" )
        """
...
class Raw(Content):
    @smart_str_validate_arguments
    def __init__(self, content: str, name: Union[Path, str]):
...
class GraphRaw(Raw, _RdfMixIn):
    ...
    def __init__(
        self,
        content: str,
        name: Union[Path, str] = Path("default"),
        content_type: GraphContentType = None,
        content_encoding: ContentEncoding = None,
    ):
...
class Url(Content):
    @smart_str_validate_arguments
    def __init__(self, url: Union[HttpUrl, str]):
        """Initializes a File object.
...
class GraphUrl(Url, _RdfMixIn):
    ...
    def __init__(
        ...
        path = "/unknown" if url.path is None else url.path

        self._validate(Path(path), content_type, content_encoding)
        super().__init__(url)


class MappingUrl(Url, _MappingMixIn):
    @smart_str_validate_arguments
    def __init__(self, url: Union[HttpUrl, str], syntax: MappingSyntax = None):
        ...
        self._validate(Path(path), syntax)
        super().__init__(url)
...
class ImportUrl(Url, _ImportMixIn):
    ...
    def __init__(
        ...
        Examples:
          ...
          >>> ImportUrl('https://example.org/data.txt',input_type=ImportType.DELIMITED, separator="\t" )
        """
        path = "/unknown" if url.path is None else url.path
